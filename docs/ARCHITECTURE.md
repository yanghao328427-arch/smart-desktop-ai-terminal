...
WebSocket 在线时不再轮询 HTTP 命令接口；WebSocket 断开后自动恢复 HTTP 命令轮询，按钮和 ACK 也退回 HTTP。当前本机 `:8091` relay 在同一端口同时提供 HTTP 和 WebSocket：它转发心跳、遥测、ACK、RFID 扫描和命令轮询，并把 ESP32 的局域网 `ws://` 连接桥接为 Hugging Face `wss://`。这样 ESP32 无需直接访问境外域名，电脑负责公网 WSS，HTTP 每 5 秒轮询只作为断线兜底。
